...
| #   | Bloc CDC §2                                  | Score initial | Après 10.6 | Après 10.7–10.16    | Après 10.18–10.30                                                    | Après Phase 11 (PR #43–#47)             | Priorité résiduelle   |
| --- | -------------------------------------------- | ------------- | ---------- | ------------------- | -------------------------------------------------------------------- | --------------------------------------- | --------------------- |
| 1   | En-tête identité                             | 3/5           | 3/5        | 4/5 ✓ (10.15)       | 4/5                                                                  | **5/5** ✓ (#43 favoris + #45 page list) | P3 (sélecteur devise) |
| 2   | Galerie média                                | 0/5           | 4/5 ✓      | 4/5                 | **5/5** ✓ (10.18 OG/Twitter)                                         | **5/5+** ✓ (#46 lightbox)               | —                     |
| 3   | Résumé factuel IA-ready (AEO)                | 3/5           | 3/5        | 4/5 ✓ (10.9)        | **5/5** ✓ (10.19 freshness pill)                                     | **5/5+** ✓ (#47 history line)           | —                     |
| 4   | Description longue                           | 3/5           | 3/5        | 4/5 ✓ (10.10)       | **5/5** ✓ (10.30 service & équipe section, >1000 mots)               | 5/5                                     | —                     |
...
**Total après soirée du 12 mai 2026 (Phase 11.7 — PR #55)** : **79/75 (~105 %)** — pas de gain sur les blocs CDC §2 (déjà au plafond) mais le filet a11y régression est passé de **4 → 12 surfaces** couvertes par axe-core (FR/EN home + 2 pages legales → + recherche FR/EN + sign-in FR/EN + sign-up FR/EN + forgot password + reservation step 1 + données MICE sur la fiche fake). Surface latente : fiche hôtel (FR + EN) en `fixme` jusqu'à résolution du `DYNAMIC_SERVER_USAGE` SSR (bug pré-existant, voir commentaire de spec). Infra `playwright.config.ts` corrigée : le `webServer.command` rebuild désormais avec des `NEXT_PUBLIC_*` placeholders pour éviter le crash d'hydratation client `Invalid environment variables`.
**Total après soirée du 12 mai 2026 (Phase 11.8 — PR #56)** : **79/75 (~105 %)** — pas de gain de score mais réparation d'un bug SSR pré-existant sur la fiche hôtel (`DYNAMIC_SERVER_USAGE` 500 sur premier render à froid). Diagnostic : la combinaison `searchParams` + `headers()` (lu par `<JsonLdScript>` pour le nonce CSP) + `revalidate = 3600` faisait jeter Next.js parce que la route ne peut pas être pré-cachée si elle touche du request-bound à chaque rendu. Correctif : `export const dynamic = 'force-dynamic'` sur `apps/web/src/app/[locale]/hotel/[slug]/page.tsx`. La suite a11y passe maintenant **15/15** (les 3 `fixme` de PR #55 sont ré-activés). Le retour à ISR est tracké comme follow-up et nécessite de sortir `<JsonLdScript>` de `headers()`.
**Total après mardi 12 mai 2026 — fin d'après-midi (Phase 11.10 — PR #58)** : **79/75 (~105 %)** — pas de gain de score, refactor architectural qui clôt définitivement l'arc CSP+JSON-LD démarré en PR #56/#57 :
...
**Total après mardi 12 mai 2026 — après-midi (Phase 11.9 — PR #57)** : **79/75 (~105 %)** — pas de gain de score, mais le filet E2E passe de **82/92 chromium** à **89/89** (zéro échec) et plusieurs bugs production latents sont corrigés :

1. **`/sitemap.xml`, `/robots.txt`, `/llms.txt`** lisaient `new URL(request.url).origin`. Combiné à `force-static`/`revalidate`, le prerender de build figait l'origine sur `http://localhost:3000`, qui partait ensuite tel quel chez Google / GPTBot / Perplexity au déploiement. Corrigé en lisant `env.NEXT_PUBLIC_SITE_URL` (même contrat que les sub-sitemaps `/sitemaps/{hotels,rooms,hubs,editorial,guides}.xml` qui étaient déjà propres).
2. **CSP nonce sur la home** : `apps/web/src/app/[locale]/page.tsx` était sur `revalidate = 3600` tout en utilisant `<JsonLdScript>` (qui lit `headers()` pour le nonce). Next.js prérendait la page avec des headers vides → nonce baked à `""` → JSON-LD `Hotel` / `FAQPage` **bloqués par CSP strict-dynamic en production** (donc aucun rich-result Schema.org pour les LLMs / Google). Correctif : `export const dynamic = 'force-dynamic'` (même patron que PR #56 sur la fiche). Trade-off : on perd l'ISR sur la home, mais on récupère les structured data côté SEO/AEO.
3. **Header `Link: agent-skills`** : le middleware émet désormais `Link: </.well-known/agent-skills.json>; rel="agent-skills"; type="application/json"` sur chaque réponse HTML (skill geo-llm-optimization §Link Header WebMCP) — point de découverte machine-readable demandé par OpenAI / Anthropic / Perplexity. Le header coexiste avec les `Link rel="alternate" hreflang` posés par next-intl (valeurs séparées par virgule, RFC 8288).
4. **5 specs E2E réalignées sur la copie actuelle** :
   - `hotel-detail.spec.ts:47` : H2 booking en mode email est `"Réservation par concierge"` (pas `"Vérifier les disponibilités"`).
   - `hotel-detail.spec.ts:79` : pareil côté EN (`"Concierge booking"`).
   - `hotel-detail.spec.ts:171` : `FAQPage.mainEntity.length` = **3** (1 question AEO `"How do I book {name}?"` injectée par la page + 2 questions éditoriales de la seed).
   - `hotel-detail.spec.ts:CTA email-mode` : la CTA n'est plus un `<a>` mais le `<form action="/reservation/start">` du `DisplayOnlyBookingCard` (teaser dates + bouton `"Demander un devis"`).
   - `api-routes.spec.ts:71` : `/llms.txt` émet des URLs absolues (`https://.../fr — …`), la regex `/\/fr$/m` devient `/\/fr\s+—/`.
   - `api-routes.spec.ts:139` : `/sitemap.xml` est un `<sitemapindex>` (pas un `<urlset>`), et chaque sub-sitemap est référencé explicitement.
   - `api-routes.spec.ts:148` : la spec CSP nonce lisait `script.getAttribute('nonce')` côté DOM, ce qui retourne `""` parce que la CSP3 spec (§"Restrict Access to the Nonce IDL Attribute") oblige les navigateurs à blanchir l'attribut après parse. Réécrite pour inspecter le HTML SSR brut via `request.get('/fr')` et croiser le nonce du header CSP avec celui des `<script type="application/ld+json">`.

Follow-ups identifiés (non bloquants) :

- 3 autres pages (`hotel/[slug]/chambres/[roomSlug]`, `destination`, `destination/[citySlug]`) sont sur `revalidate = 3600` et utilisent `<JsonLdScript>` — même classe de bug CSP que la home. Pas couvertes par un test E2E aujourd'hui mais à passer en `force-dynamic` (ou à refactorer pour faire passer le nonce en prop) lors d'un prochain pass.
- Le refactor `<JsonLdScript>` qui prendrait `nonce` en prop reste le bon chemin pour réactiver ISR partout sans perdre la CSP. Tracké.

La fiche dépasse toujours le seuil de publication face à Booking / Mr & Mrs Smith / AOR Hotels sur **15 blocs sur 15**. Reste uniquement : finitions polish (favoris déjà livrés en PR #51 + bouton remove, B2B/MICE en PR #53).
...
| Migration                          | Colonnes ajoutées                                                                    | Bloc CDC | Prio | Statut                    |
| ---------------------------------- | ------------------------------------------------------------------------------------ | -------- | ---- | ------------------------- |
| `0020_hotel_phone_e164_column.sql` | `phone_e164 text` (E.164 format, optional) — exposé en JSON-LD `telephone`           | §2.15    | P3   | ✓ Phase 10.29 #38         |
| `0021_user_favorites.sql`          | nouvelle table `public.user_favorites` (user_id, hotel_id, created_at, RLS own-only) | §2.1     | P2   | ✓ Phase 11.1 #43          |
| `0022_hotel_dates_columns.sql`     | `opened_at date`, `last_renovated_at date` (CHECK 1500-current_date + ordering)      | §2.15    | P3   | ✓ Phase 11.2 #47          |
| `0023_hotels_google_reviews.sql`   | `google_reviews jsonb` (5-10 quotes Google Places, ingest cron J-7)                  | §2.10    | P2   | ouvert (bloc 10 résiduel) |
