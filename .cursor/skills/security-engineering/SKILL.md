...
### CSP nonce ↔ JSON-LD ↔ force-dynamic contract

The production CSP is **strict CSP3 with `'strict-dynamic'`**:

```
script-src 'self' 'nonce-{nonce}' 'strict-dynamic'
```

No `'unsafe-inline'` fallback. Three coupled rules apply to every page
that emits any inline `<script>` (including JSON-LD):

1. **Use the shared wrapper** `<JsonLdScript data nonce />` from
   `apps/web/src/components/seo/json-ld.tsx`. Never emit raw
   `<script type="application/ld+json">` — it bypasses the nonce prop.
2. **Read the nonce at the page boundary**, once:
   `const nonce = (await headers()).get('x-nonce') ?? undefined;`. Do NOT
   call `headers()` inside the leaf component (paid as PR #56 hotel
   detail `DYNAMIC_SERVER_USAGE` 500s and PR #57 home empty-nonce ISR
   caching).
3. **The page MUST be `force-dynamic`** (`export const dynamic = 'force-dynamic'`)
   so the per-request nonce is generated by middleware and the HTML is
   never cached with a stale or empty value.

The middleware (`apps/web/src/middleware.ts`) generates the nonce per
request and exposes it via the `x-nonce` request header.

Cross-skill: `structured-data-schema-org` §CSP-nonce-contract,
`nextjs-app-router` §JSON-LD-and-dynamic-rendering.
...
- **`structured-data-schema-org`** — JSON-LD wrapper that consumes the nonce.
- **`nextjs-app-router`** — force-dynamic requirement on JSON-LD pages.
